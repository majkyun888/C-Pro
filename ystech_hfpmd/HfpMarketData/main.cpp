...
RegisterOption(HFP_ID, "", "HFP ID")
RegisterOption(HFP_LICENSE, "", "HFP LICENSE")
RegisterOption(HFP_FRONT_IP, "", "HFP FRONT IP")
RegisterOption(HFP_FRONT_PORT, "", "HFP FRONT PORT")
...
execute_type execute(16, 10, tars::util::micro_spin());
os::auto_file af;
class  HfpData
{
public:
	HfpData(quotation_data& data)
	{
		...
		strcpy(marketId, data.marketId); 
		open = data.open / PRICE_UNIT;
		high = data.high / PRICE_UNIT; 
		low = data.low / PRICE_UNIT; 
		_new = data._new / PRICE_UNIT; 
		last_close = data.last_close / PRICE_UNIT; 
		average = data.average / PRICE_UNIT;
		total_volume = data.total_volume;
		total_amount = data.total_amount; 
		subs_volume = data.subs_volume; 
		cur_volume = data.cur_volume; 
		type = data.type;
		down_limit = data.down_limit / PRICE_UNIT; 
		up_limit = data.up_limit / PRICE_UNIT; 
		balance_price = data.balance_price / PRICE_UNIT; 
		updown_base = data.updown_base / PRICE_UNIT;
...
namespace hfp_data
{
	...
	void PrintQuoteHeader() {
		std::string CSVTitle("Seq,DateTime,Raw_Time,Contract_Name,MarketId,Open_Price,Highest_Price,Lowest_Price,"
			...
			"SP1,SP2,SP3,SP4,SP5,SV1,SV2,SV3,SV4,SV5,Start_Time,End_Time,Offset_Time\n");

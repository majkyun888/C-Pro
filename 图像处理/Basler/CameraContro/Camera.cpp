...
extern UINT             trigger_label[NUMTRIGGER+2];//Pour added for MEP colordot inspection on 20191118
extern UINT             lng;
...
{
	_im_gray = (BYTE*)_aligned_malloc( dimx*dimy,16 ); //Pour added 20180301
	//(BYTE*) im_gray_data = (BYTE*) malloc(dimx*dimy); //Pour removed 20180301
	ASSERT( _im_color != NULL );
	ASSERT( _im_gray && AfxIsValidAddress(_im_gray,dimx*dimy) );
	ZeroMemory( _im_gray, dimx*dimy );

	// transform RGB to gray  

	for( int y = 0; y < dimy; ++y )
	{
		BYTE* pline = _im_color + 3*( y*dimx );
		for ( int x = 0; x < dimx; ++x )
		{
			double R = *(pline + 3*x+2);
			double G = *(pline + 3*x+1);
			double B = *(pline + 3*x+0);
			...
			*(_im_gray + y* dimx + x) = (BYTE)(R*0.25+G*0.625+B*0.125);// according to Basler document
		}
	}
...
//  int ind = 0;
  ...
  ASSERT( ind>=0 && ind<num_tlc );
  ASSERT( camera[ind].triglab>=0 && camera[ind].triglab<NUMTRIGGER+2 );
...
UINT __cdecl inspect_3d_cosmetic( LPVOID pParam )
{
  ...
  catch (GenICam::GenericException &e)
  {
    ...
    //TRACE( "Genicam exception thrown <%s>\n", e.GetDescription() );
    swprintf( msg_err, 200, L"Genicam exception thrown <%s>\n", AtoW(e.GetDescription()) );
			  //add by yang 20180323
	  //it only detects the disconnection when there is vial passing 
	  	  alarm_data->on   = TRUE;
 ...
 //	//Pour marked on 20190430 for previous setting of first C20 machine
 // #ifdef TIVS_C20
 //   if ( IS_PC1 )
 //   {
...
UINT __cdecl inspect_3d_singlecam( LPVOID pParam )
{
  ...
  ASSERT( bottom_insp[ind] || focolor_insp[ind] || oralfo_insp[ind] || drcolor_insp[ind] );
...
UINT __cdecl inspect_3d_doublecap( LPVOID pParam )
{
  ...
  ADS_data_type ADS_insp_res;
  char name[50];
  ASSERT( camera[ind].triglab>=0 && camera[ind].triglab<25 );
  sprintf( name, "PRODUCT_FLOW.INSP_RES[%d]", camera[ind].triglab );
  if ( ADS_init_parameter( name, &ADS_insp_res, sizeof(insp_res_type) ) )
	{
    txt.Format( L"Error in getting handle of <%s>", name );
  ...
  insp_res_type* insp_res = (insp_res_type*) ADS_insp_res.val;
  #ifdef DOTRING_COLOR
  ADS_data_type ADS_insp_res2;
   sprintf( name, "PRODUCT_FLOW.INSP_RES[%d]", camera[ind].triglab2 );
  if ( ADS_init_parameter( name, &ADS_insp_res2, sizeof(insp_res_type)) )
	{
    txt.Format( L"Error in getting handle of <%s>", name );
  	Message_box( msg, MBOX_ERROR );
	}
  insp_res_type* insp_res2 = (insp_res_type*) ADS_insp_res2.val;
#endif
  ...
  #ifdef DOTRING_COLOR
  UINT          bit_drc_ok = BIT_DRCOLOR1;   // TC1
  if ( ind==1 ) bit_drc_ok = BIT_DRCOLOR2;   // TC2
  if ( ind==2 ) bit_drc_ok = BIT_DRCOLOR3;   // TC3
#endif
...
//============		
          ...
          {
            timer.reset();
            //if ( image_dsp[ind]->visible && !show_back_light ) 	
			//	image_dsp[ind]->scale_grabresult( ptrGrabResult ); // TOPLIGHT to be displayed //should not change data which ptr_dsp point to, if show back light
            ...
            if ( tip_insp[ind] )
            {
              ...
              if (tip_insp[ind]->Process_image( ptrGrabResult, 1 ))//any one =0 =>no reject - NIMG=1 => TOPLIGHT INSPECTION
              {
                toplight_reject = TRUE;
                top_rej++;
              }
              ...
              if ( image_dsp[ind]->visible && !show_back_light ) // TOPLIGHT to be displayed
              {
                ...
                camera[ind].text[2]->SetText( tip_insp[ind]->txt2, tip_insp[ind]->col2 );
                  ...
                  camera[ind].text[4]->SetText( tip_insp[ind]->txt3, tip_insp[ind]->col3 );
                #endif
				  SCALE_IMG( ind, image_dsp[ind]->ptr_buf );//Pour added on 20191123
                image_dsp[ind]->Draw_image();
              }
            ...
            if (  toplight_reject )//toplight_reject ||
            {
              insp_res2->val = 0;          // nothing: reject
              update_view = TRUE;
              camera[ind].text[3]->SetText( txt, TXT_DARKRED );  
            }
            else
            {
              insp_res2->val = bit_drc_ok; // set good result bit high
              camera[ind].text[3]->SetText( txt, TXT_DARKGREEN );  

            }

            insp_res2->num = trigger_label[camera[ind].triglab];
            ADS_write_parameter( &ADS_insp_res2 );
            nimg_acq++;
          ...
          else // BACKLIGHT INSPECTION (FIRST)
          {
			...
			if ( image_dsp[ind]->visible && show_back_light ) 
				image_dsp[ind]->scale_grabresult( ptrGrabResult ); //SCALE_IMG( ind, img_gray ); BACKLIGHT to be displayed,color image not BW 
            if ( tip_insp[ind] )
            {
              back_insp++;
              //tip_insp[ind]->tl_drawovl = FALSE;
              tip_insp[ind]->bl_drawovl = image_dsp[ind]->visible && show_back_light;
              if ( tip_insp[ind]->Process_image( ptrGrabResult, 0 ) ) // !=0  reject - NIMG=0 => BACKLIGHT INSPECTION
              {
                backlight_reject = TRUE;
                back_rej++;
              }
			...
			if (  backlight_reject )//toplight_reject ||
            {
              insp_res->val = 0;          // nothing: reject
              //update_view = TRUE;
              //camera[ind].text[3]->SetText( txt, TXT_DARKRED );  
    ...
    try // Genicam exception handling
    {
      ...
      if ( camera[ind].camera->RetrieveResult( 1000, ptrGrabResult, TimeoutHandling_Return  ) )
      {
        ...
        if ( ptrGrabResult->GrabSucceeded() )
        {
          ...
          if ( tip_insp[ind] )
          {
			cv::Mat img_in(ptrGrabResult->GetHeight(), ptrGrabResult->GetWidth(), CV_8U, static_cast<unsigned char*>(ptrGrabResult->GetBuffer()), ptrGrabResult->GetWidth());//Pour 20191116
...
//    if ( saveinsp_on && ind==(IS_PC1?3:0) )  // index for PC1/PC2
  ...
  catch (GenICam::GenericException &e)
  {
    ...
    if ( logger )
    {
      swprintf( msg_err, 200, L"inspect_3d_particle[%s] - genicam exception thrown <%s>\n", camera[ind].name, AtoW(e.GetDescription()) );
...
////Pour marked on 20190505 for first C20 machine
//    #ifdef TIVS_C20
//      bit_particle_ok = BIT_COSMETIC;    // PC2 - part3/cosm
//      bit_fillingl_ok = BIT_FILLINGL2;   // PC2 - part2
//    #endif
...
//Pour changed on 20190505
    #ifdef TIVS_C20
      bit_particle_ok = BIT_PARTICLE2;   // PC2 - part3/cosm
      bit_fillingl_ok = BIT_FILLINGL1;   // PC2 - part2 //Pour marked on 20190509
    #endif
//Pour changed on 20190505
  }
      ...
      while ( nimg_acq < numimg && !skip_lost )
      {
        ...
        if ( camera[ind].camera->RetrieveResult( 8000, ptrGrabResult, TimeoutHandling_Return  ) )
        {
          ...
          if ( ptrGrabResult->GrabSucceeded() )
          {
            ...
            //if ( particle_insp[ind] )              particle_insp[ind]->Process_image( img_acq, nimg_acq );     //Pour removed 20180601
...
//          image_dsp[ind]->Draw_image(TRUE);
        ...
        #endif
/*
	//Pour marked for old version on 20190529
	...
	//Pour marked for old version on 20190529
*/
		  	//Pour marked for new version on 20190529
        if ( datarec_on && result )
        {
          //txt.Format(L"inspect_3d_particle[%d]",ind);
          ASSERT( triggerold>=0 && triggerold<60 );
          ...
          if ( result2 )
          {
            if ( ind<4 ) result ->InsertI( particle_insp[ind]->part_err/*totnpt*/, triggerold ); // particle2 into result  //Pour marked for GPO project on 20190601 //totnpt
            else         result2->InsertI( particle_insp[ind]->part_err/*totnpt*/, triggerold ); // particle3 into result2 //Pour marked for GPO project on 20190601 //totnpt
          }
          else result->InsertI( particle_insp[ind]->part_err/*totnpt*/, triggerold ); // only particle2 //Pour marked for GPO project on 20190601 //totnpt
...
UINT __cdecl alarm_warning_3d( LPVOID pParam )
{
  ...
  while ( !CHK_BIT(thread_exit, 1) )
  {
    timer.reset();
  	...
  	GetLocalTime( &time );
    swprintf( str, 200, L"%02d/%02d/%04d %02d:%02d:%02d", time.wDay, time.wMonth, time.wYear, time.wHour, time.wMinute, time.wSecond );

...
//  #ifdef SAVE_FULL_PROC
 ...
 // void ( CImage_display::*dsp_to_buf  )( POINT* );          //function pointer
  ...
  //void      buf2mat( CGrabResultPtr ptr_grab, cv::Mat& im_in );
  void      buf2mat( CGrabResultPtr ptr_grab, cv::Mat& im_in );//color raw Bayer data to RGB data, output to Mat 
  void      bayer2rgb( CGrabResultPtr ptr_gr );//color raw Bayer data to RGB data, output to ptr_buf 

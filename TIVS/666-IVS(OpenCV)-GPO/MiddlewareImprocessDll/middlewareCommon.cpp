...
void MiddlewareImProcess::imsOnlineSave(bool good, CString _cstr_CamID, cv::Mat imRlt)
{
	...
	if (_cstr_CamID == _T("")) {
		...
		/*sprintf(chParaXml, "%s\\para.xml", RootImageSaveFolder().c_str());*/
		sprintf(chParaXml, "%s\\", RootImageSaveFolder().c_str());
		MakeSureDirectoryPathExists(RootImageSaveFolder().c_str());
	}		
	else {
		/*sprintf(ch, "%s\\%s\\%s_%s_%04d\\", RootImageSaveFolder().c_str(), Unicode2StdString(_cstr_CamID).c_str(), str_time.c_str(), str_good.c_str(), countImsOnSave++);*/
		...
		/*sprintf(chParaXml, "%s\\%s\\para.xml", RootImageSaveFolder().c_str(), Unicode2StdString(_cstr_CamID).c_str());*/

...
CFOcolor_inspection::CFOcolor_inspection( UINT ind, CFile_mngmt* file_manager_in, int mode_in )
{
...
CFOcolor_inspection::~CFOcolor_inspection(void)
{
...
int CFOcolor_inspection::Set_inspection_area( int ind )
{
...
//  if ( area && area->m_hWnd ) area->ShowWindow( SW_SHOW );
  ...
  TRACE("CFOcolor_inspection::Set_inspection_area(%d)\n", ind);
  return( 0 );
}



int CFOcolor_inspection::Add_menu_parameters( CContainer_wnd* child )
{

  pView->menu_btn[pView->num_menu_btn] = new CMenu_button( string_2400[lng], child );
  ...
  pView->parameter[pView->num_params++] = new CParameter( &maxerr_lab,    CTYPE_DINT,  0, pView->menu_btn[pView->num_menu_btn]->child, string_2410[lng], string_unit[lng],  100,    0,  1000, file_manager );
	...
	pView->parameter[pView->num_params++] = new CParameter( &maxerr_col2,    CTYPE_DINT,  0, pView->menu_btn[pView->num_menu_btn]->child, string_2412a[lng], string_unit[lng],  100,    0,999000, file_manager );
	#endif
...
int CFOcolor_inspection::Process_image( CGrabResultPtr ptr_gr, UINT nimg )
{
...
//int CFOcolor_inspection::Process_image( BYTE* img_acq, UINT nimg )
int CFOcolor_inspection::Process_image( BYTE* ptr_in, UINT nimg )
{
//  int x, y;
...
int CFOcolor_inspection::Analyze_center( void )
{
  BYTE* ofs;
  int   r, i;
  int err = 0;
...
double CFOcolor_inspection::funz_t( double in )
{
  if ( in > 0.008856 ) return( pow( in , 1.0/3.0 ) );
  else return( 7.787 * in + 16-0 / 116.0 );
}

int CFOcolor_inspection::Calculate_average( void )
{
/*  int x, y;
  totr = 0;
  totg = 0;
...
int CFOcolor_inspection::Calculate_blob( void )
{
...
int CFOcolor_inspection::Search_center( void )
{
  int    i;
  int    x      = xcnt;
  int    y      = ycnt;
...
int CFOcolor_inspection::Calculate_scores( int x, int y )
{
...
UINT CFOcolor_inspection::sectbrgth( int xc, int yc, int side ) // 0=right 1=bottom 2=left 3=top 
{
  UINT diff = 0;
  int  i;

...
class CDRcolor_inspection
{

//	DECLARE_DYNAMIC(CDRcolor_inspection)

public:
  CDRcolor_inspection( UINT ind, CFile_mngmt* file_manager_in=NULL, int mode_in=TIP_INSPECTION );
  ~CDRcolor_inspection(void);
...
/*  // accumulators for calculating avergae colors
  ...
  int npt_cam; // single camera count 

  int insp_label; // for synchronization, inspection label just used

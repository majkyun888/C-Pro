...
class CFOcolor_inspection
{

//	DECLARE_DYNAMIC(CFOcolor_inspection)

public:
  CFOcolor_inspection( UINT ind, CFile_mngmt* file_manager_in=NULL, int mode_in=TIP_INSPECTION );
  ~CFOcolor_inspection(void);
...
//  int radius; // radius used for alignment
  ...
  int     maxerr_col2;    // maximum allowed color error (spots/flaws)
#endif

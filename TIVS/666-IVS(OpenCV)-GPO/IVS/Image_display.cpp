...
void CImage_display::buf2Mat( BYTE* ptr_buf, cv::Mat img, int type )
{
  if ( type )
  {
	// if color image data, RGB should be transferred to BGR for storing in cv::Mat
	Mat img = Mat::zeros( dim_buf.cy, dim_buf.cx, CV_8UC3 );  
	for(int y = 0; y < dim_buf.cy; ++y)
	{
		BYTE* pline = ptr_buf + 3*( y*dim_buf.cx );
		BYTE* pline_tmp = img.data + 3*( y*dim_buf.cx );
		for ( int x = 0; x < dim_buf.cx; ++x)
		{
			*(pline_tmp+3*x+0) = *(pline + 3*x+2);
			*(pline_tmp+3*x+1) = *(pline + 3*x+1);
			*(pline_tmp+3*x+2) = *(pline + 3*x+0); 
		}
	}
    //img_in.data = temp;
	//Mat img_in(dimy, dimx, CV_8UC3, temp );  
	//cv::Mat img(dim_buf.cy, dim_buf.cx, CV_8UC3, temp );  
  }
  else 
	cv::Mat img(dim_buf.cy, dim_buf.cx, CV_8U, ptr_buf );  
...
//  ptr_dsp[((x*scale)>>10)+(dim_dsp.cy-1-((y*scale)>>10))*dim_dsp.cx] = col; // //  ptr_dsp[x+y*dim_dsp.cx] = col;
    ...
    if( x== 2047 && y == 0) 
	  y = 0;
  int temp = dim_dsp.cx-1-((y*scale)>>10)+(dim_dsp.cy-1-((x*scale)>>10))*dim_dsp.cx ;
  if( temp < 0) temp = 0;

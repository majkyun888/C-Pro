...
CStopper_inspection::CStopper_inspection( UINT ind, CFile_mngmt* file_manager_in, int mode_in )
{
...
CStopper_inspection::~CStopper_inspection(void)
{
...
/*
int CStopper_inspection::Allocate_profile( void )
{
...
int CStopper_inspection::Set_inspection_area( int ind )
{
...
//  if ( area && area->m_hWnd ) area->ShowWindow( SW_SHOW );
  ...
  TRACE("CStopper_inspection::Set_inspection_area(%d)\n", ind);
  return( 0 );
}



int CStopper_inspection::Add_menu_parameters( CContainer_wnd* child )
{
...
int CStopper_inspection::Process_image( BYTE* img_acq, UINT nimg )
{
  int x, y;
...
int CStopper_inspection::Bottom_analysis_circle( void )
{
  err_pos      = 0;
  err_neg      = 0;
  err_max      = 0;
...
int CStopper_inspection::Bottom_double_analysis( void )
{
...
int CStopper_inspection::Bottom_analysis( void )
{
  err_pos      = 0;
  err_neg      = 0;
  err_max      = 0;
...
int CStopper_inspection::Search_center( void )
{
...
int CStopper_inspection::Search_center2( void )
{
...
int CStopper_inspection::Calculate_scores( int x, int y )
{
...
// 0=center - 1=right - 2=bottom - 3=left - 4=top
int CStopper_inspection::Calculate_scores( int x, int y, int r )
{
  // central position's score
...
/*
int CStopper_inspection::Calculate_score( int ind )
{
  ASSERT( ind>=0 && ind<5 );
...
UINT CStopper_inspection::sectbrgth( int xc, int yc, int side ) // 0=right 1=bottom 2=left 3=top 
{
  UINT lum = 0;
  int  i;
...
UINT CStopper_inspection::sectbrgth( int xc, int yc, int rc, int side ) // 0=right 1=bottom 2=left 3=top 
{
  UINT lum = 0;
  int  i;

...
BEGIN_MESSAGE_MAP(CUser_manager, CDialog)
  ...
  ON_EN_CHANGE(IDC_TXTUSERNAME2, &CUser_manager::OnEnChangeTxtusername2)
END_MESSAGE_MAP()
...
void CUser_manager::OnShowWindow(BOOL bShow, UINT nStatus)
{
  ...
  CString txt;
  txt.Format( L"%s", string_3835[lng] );
  ((CButton*) GetDlgItem( ID_UPDATE  ))->SetWindowText(txt ); // >SetWindowText( L"Operator level"    );
  txt.Format( L"%s", string_3836[lng] );
  ((CButton*) GetDlgItem( ID_ADD ))->SetWindowText(txt); // >SetWindowText( L"Operator level"    );
  txt.Format( L"%s", string_3837[lng] );
  ((CButton*) GetDlgItem( ID_DELETE  ))->SetWindowText(txt); // >SetWindowText( L"Operator level"    );
  txt.Format( L"%s", string_3838[lng] );
  ((CButton*) GetDlgItem( IDCANCEL   ))->SetWindowText(txt); // >SetWindowText( L"Operator level"    );

  //Pour added to show the access information for LanZhou Shengwosuo on 20200917
  GetDlgItem( IDC_TXTUSERNAME2 )->SetWindowText( info_accessinfo_chn1[lng] );
  GetDlgItem( IDC_TXTUSERNAME3 )->SetWindowText( info_accessinfo_chn2[lng] );
  GetDlgItem( IDC_TXTUSERNAME4 )->SetWindowText( info_accessinfo_chn3[lng] );
...
void CUser_manager::OnEnChangeTxtusername2()
{

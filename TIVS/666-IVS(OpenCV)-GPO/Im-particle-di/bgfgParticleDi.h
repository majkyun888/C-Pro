...
namespace algoParticleDi {
	...
	public:
		...
		~BackgroundSubtrator();
		void buildmodel(cv::InputArray _image, cv::InputArray _mask, bool _en_dy,bool _needToInitialize); // 20180525 监控背景的动态程度
	...
	private:
		virtual void initialize(cv::Size _frameSize, int _frameType);
		cv::Size frameSize;
		int nTrain;
		float svar0; //起始标准差
		float sminVar; //最大标准差

...
void Manange::collectImgPaths(CString _cstrRoot, CString _cstrRootCsv)
{
...
bool Manange::procOnce(aPara _para, ParaInter _para_inter,bool _toNext, int _camIdx, bool& _rlt)
{
	if (true == _toNext)
		itCurPath++;
	...
	if (itCurPath == vecPath.end()) {
		itCurPath = vecPath.begin();
	/*	MessageBox(NULL, _T("图像序列容量异常，返回结果可能异常"), NULL, NULL);*/
		return false;
	}
...
void Manange::ims2write(CString _path, vector<cv::Mat> _srcs, vector<cv::Mat>& _rlts,bool _good, string _strIdx/*, int _idx*/)
{
	...
	int thickness = 3;
	cv::Scalar sclr = cv::Scalar(255, 0, 0);
	cv::putText(imrlt, _strIdx, cv::Point(0, imrlt.rows / 2), fontFace, fontScale, sclr, thickness, 8);
	cv::imwrite(Unicode2StdString(str), imrlt);

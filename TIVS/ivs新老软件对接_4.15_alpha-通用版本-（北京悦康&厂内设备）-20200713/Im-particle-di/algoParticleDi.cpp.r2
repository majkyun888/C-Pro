...
namespace algoParticleDi {
	...
	void algoParticleDi::ProcParticle::findSideVec(vector<cv::Mat> & __vt_Im, vector<ProBottltBase> & _vtProBase, vector<int> & _vtLen)
	{
		...
		for (int i = 0; i < __vt_Im.size(); i++) {
			...
			if (m_particle_type == particle_di) {
				...
				int thr_mea = 15;
				uchar mea = std::min(meaL, meaR) - thr_mea;
				cv::reduce(imcolrange, mbin, 1, CV_REDUCE_MIN);
				cv::threshold(mbin, mbin, mea/*m_prePara.thr_side_br*/, 255, THRESH_BINARY_INV);  // 不应该是固定值，而是通过直方图可以直接检测出来
			}
			int pyr = 2;
	...
	cv::Rect ProcParticle::CalNewRoi(vector<cv::Mat>& _vmIn) {
		...
		//if ( roiEn.br().y + hgtEn < _vmIn.at(0).rows && mtRef.empty() ) {
		//	mtRef = _vmIn.at(0).rowRange(roiEn.br().y, roiEn.br().y + hgtEn).colRange(roiEn.tl().x, roiEn.br().x);
		//}
		//if (roiEn.tl().y > 5 && mtRef.empty() ) {
		//	mtRef = _vmIn.at(0).rowRange(roiEn.tl().y - hgtEn > 0 ? roiEn.tl().y - hgtEn : 0, roiEn.tl().y).colRange(roiEn.tl().x, roiEn.br().x);
		//}
		//if (!mtRef.empty()) {
		//	cv::reduce(mtRef, mtRef, 0, CV_REDUCE_AVG);
		//	cv::repeat(mtRef, roiEn.height, 1, mtRef);
		//}
	...
	bool ProcParticle::ProcGlobal(vector<cv::Mat> & _vmIn/*, VMCollect& _vm_collect*/, vector<cv::Rect>& _rtMask/*cv::Mat _maskBlink*/)
	{
		...
		if (m_particle_type == particle_bei) {
			...
			m_proBottleSpec.roi = cv::Rect(cv::Point(m_proBottleSpec.roi.tl().x > m_proBottleSpec.scanBorder ? m_proBottleSpec.roi.tl().x: m_proBottleSpec.scanBorder, m_proBottleSpec.roi.tl().y) , m_proBottleSpec.roi.br() ); //
		}			

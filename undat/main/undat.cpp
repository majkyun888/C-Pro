...
void getFiles( Writer * opert)
{
	string ce;
	ifstream fin;
	Data_File * tp = nullptr;
	string temp;
	vector<string> vec;
	fin.open("C:/Users/liu.bingqiang/Desktop/path.txt",ios::in);
	while (!fin.eof())
	{
	...
	while (ite != vec.end())
	{
		...
		idx = file_name.find("TickAB");
		if (file_name.find("TickAB") != string::npos)
		{
			 tp = new TickAB_data;
		}
		else if (file_name.find("Transaction") != string::npos)
		{
			 tp = new Transaction_data;
		}
		ce = split(file_name, "//");
		ce += ".csv";
		cout << ce << endl;
		opert->start_writer(&tp);
		opert->Open_file(file_name.c_str(), "rb");
		opert->write_head(ce.c_str());
		opert->write_file(); 
		ite++;
	}
}
...
int main()
{
	...
	opertt->Open_file("C:/Users/liu.bingqiang/Desktop/510050.dat", "rb");
	opertt->write_head("./510050.csv");
	opertt->write_file();*/
	getFiles(opertt);
